...
int main()
{
	...
	cout << "Enter neuron addition probability (only matters for feedforward with added neurons): " << endl;
	cin >> prob;
	cout << "Enter Number of Modules" << endl;
	cin >> numModules;
	cout << "Enter '0' for feedforward, '1' for feedback" << endl;
	cin >> FB;
	...
	out << "vector<bool> damage;" << endl;
		for(int i=0;i<nAx;i++)
		{
			...
			{
				if(ei == 0)
				{ // FEEDBACK NO EXTRA CELL, DAMAGE ON 0mod3
					if(i%3 == 0)
					{
						out << "damage.push_back(true);"<<endl;
					}
				...
				{ // FEEDBACK EXTRA CELL, 1mod4 GETS DAMAGE
					if(i%4 == 1)
					{
						out << "damage.push_back(true);"<<endl;
					}
